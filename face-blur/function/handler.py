...
def blur_faces_opencv(image, blur_factor=20):
    """ Blur faces in an image using OpenCV cascade classifier for face detection. Input is a PIL image."""
    ...
    for (x, y, w, h) in faces:
        ...
        blurred_face = cv2.GaussianBlur(face, (99, 99), blur_factor)
...
def blur_faces_mtcnn(image):
    """ Blur faces in an image using MTCNN for face detection. Input is a PIL image."""
    # Convert the image to OpenCV format (numpy array)
    img_array = np.array(image.convert("RGB"))

    # Create an instance of MTCNN for face detection
    detector = MTCNN(scale_factor=0.819, min_face_size=15, steps_threshold = [0.55, 0.65, 0.65])
    ...
    for face in faces:
        x, y, w, h = face['box']
        face_array = img_array[max(0, y):y+h, max(0, x):x+w]
        blurred_face = cv2.GaussianBlur(face_array, (99, 99), 30)
        img_array[max(0, y):y+h, max(0, x):x+w] = blurred_face
...
def handle(req):
    ...
    try:
        ...
        print(f"Error creating bucket - {str(e)} Using {minio_api} as api." )
        return
    ...
    try:
        ...
        modified_image, num_faces = blur_faces_mtcnn(image)
        ...
        print(f"Sucess! There were {num_faces} face(s) detected and blurred.")
